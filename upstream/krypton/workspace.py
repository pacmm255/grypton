...
Each target gets ``TARGETS_DIR/<slug>/`` containing the documents the brief
...
def _file_lock(lock_path: Path):
    ...
    f = os.open(str(lock_path), os.O_CREAT | os.O_RDWR, 0o644)
    try:
...
class TargetMeta:
    ...
    worker_kind: str = ""            # "claude" | "codex"; empty => claude
    manager_session_id: str = ""     # codex session id, for resume
    fallback_manager_session_id: str = ""  # claude fallback manager session id
    worker_model: str = ""           # per-target override; empty => config default
    manager_kind: str = ""           # "codex" | "claude"; empty => config default
    manager_model: str = ""          # per-target Codex model override; empty => config default
    manager_effort: str = ""         # per-target Codex effort override; empty => config default
...
class Workspace:
    ...
    def create(self, target: str, target_type: str = "auto") -> TargetMeta:
        for d in (self.root, self.research_dir, self.scripts_dir, self.loot_dir,
                  ...
                  self.claude_dir, self.root / ".ledger"):
    ...
    def set_severity_verdict(self, finding_id: str, verdict: dict) -> Optional[dict]:
        hit = self.findings.update(finding_id, lambda r: r.__setitem__("manager_verdict", verdict))
        if hit:
    ...
    def _fmt_finding(r: dict) -> str:
        ...
        if r.get("status") == "suppressed-by-scope":
            head += "  _(suppressed by scope)_"
        lines.append(head)
