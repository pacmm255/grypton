...
class MockWorker:
    ...
    async def run_turn(self, directive: str) -> TurnResult:
        ...
        if self.on_event:
            if self.counter == 1:
                self.on_event({"type": "system", "subtype": "init", "model": "mock-opus-4-8",
                               "tools": ["Bash", "Read"], "mcp_servers": [{"name": "krypton"}],
                               "session_id": "mock"})
    ...
    def _default_script(self, directive: str) -> str:
        ...
        i = self.counter
        if "fabtest" in d:
            return "Wrote results to /root/krypton/targets/does-not-exist/ghost.md (412 lines)."
