...
| Persistent worker and manager context | One OpenCode session ID per role is resumed across turns and stored in `target.json`; static system instructions live in the OpenCode agent prompt instead of being duplicated in every persistent user turn; each role has private XDG state |
...
| Native and structured tool calling | OpenCode native read/search tools plus the local `grypton` MCP server; Bash, write/edit, and native web tools are denied, bounded offline analysis is exposed through `local_analyze`, every structured call is appended to `.ledger/tool-calls.jsonl`, and potentially effectful calls receive a durable pre-dispatch restart marker |
...
| Finding candidates meet an evidence gate | The `record_finding` MCP schema and dispatcher require title, severity, class, affected surface, impact description, reproduction, and concrete evidence; prompts keep incomplete, P5 informational, and program-excluded observations in surface/tested ledgers |
