...
| Every new finding is independently validated with Astra max | Fresh tool-disabled `codex exec` process per finding in `grypton/providers.py`; exact model `gpt-6-astra`, effort `max`; a fair bounded allocator includes every explicitly referenced artifact without letting a large early capture starve later controls |
...
| Continuous autonomous operation | `grypton/engine.py` runs worker → manager → independent validator cycles until an explicit ceiling, operator stop, binding scope boundary, or unrecoverable repeated fault |
| Working `init --target` and positional target CLI | `grypton/cli.py`; both `grypton init --target HOST` and `grypton init HOST` start immediately; review commands (`show`, `findings`, `surface`, `history`, `scope`, `audit`, `report`) and run/tool/dashboard commands are available |
...
| Finding state reflects validation | `Workspace.set_severity_verdict` maps Astra results to confirmed, needs-more-evidence, rejected, or validation-pending state; CLI and UI separate confirmed results from candidates |
...
- Findings remain unconfirmed until an Astra verdict is persisted.
